...
v0.5.1은 정식 릴리즈입니다. 보호·복원 회귀와 실제 Windows 검증을 거쳤지만, 기업 도메인/MDM·IPv6 DNS 등 모든 구성을 검증한 것은 아닙니다. 성능 측정의 조건과 남은 한계는 해당 릴리즈 노트를 참고하세요. 설치 파일은 아직 Authenticode 서명이 없어 Windows에 알 수 없는 게시자가 표시될 수 있습니다. 파일 해시는 코드 서명을 대신하지 않습니다. 공식 배포 경로에서 받은 파일만 실행하세요.
